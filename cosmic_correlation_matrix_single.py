...
class correlation(object):
    ...
    def compute_correlation_matrix(self): 
        self.wpcov_single_w = self.wpcov_w[np.random.randint(self.wpcov_w.shape[0])]
        self.wpcov_single_wo = self.wpcov_wo[np.random.randint(self.wpcov_wo.shape[0])]
        self.sigma_wp_w = np.sqrt(self.wpcov_single_w.diagonal())
        self.sigma_wp_wo = np.sqrt(self.wpcov_single_wo.diagonal())
        self.all_comparison = (self.wpcov_single_w/self.sigma_wp_w).T/self.sigma_wp_w
        self.half_wo = (self.wpcov_single_wo/self.sigma_wp_wo).T/self.sigma_wp_wo
